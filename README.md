...
- **EOA-like signature lifecycle.** Signatures have no expiry by default; a pending signature is cancelled by consuming its nonce, exactly like replacing a pending EOA transaction. The nonce is consumed before any external call, so a signature cannot be replayed by reentering from a target.
