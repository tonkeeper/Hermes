...
contract HermesDelegateV1 is HermesBase, IAccount, IERC1271, IERC7821, IERC5267 {
    ...
    /// @notice The preferred ERC-4337 EntryPoint for discovery. Both v0.8 and v0.9 are trusted as
    ///         callers; this reports the newest (v0.9). Probe the full set with `isSupportedEntryPoint`.
    function entryPoint() public pure returns (address) {
        return ENTRY_POINT_V9;
    }
    ...
    ///         batch early once the call succeeds, log-and-continue while it fails). The exec type
    ///         and policies are part of `mode` and therefore bound into the signed digest.
    ...
    ///      BREAK_ON_SUCCESS success ends the batch early. Try batches are capped at
