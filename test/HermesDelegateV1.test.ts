...
const ENTRY_POINT_V9_ADDR = ENTRY_POINT_ADDR; // what entryPoint() reports (preferred)
...
describe("HermesDelegateV1 (EIP-7702 + ERC-4337 + ERC-1271)", () => {
    ...
    describe("constructor & pure views", () => {
        ...
        it("entryPoint() returns the canonical EntryPoint v0.9 address", async () => {
            const { delegate } = await setup();
            expect(await delegate.entryPoint()).to.equal(ENTRY_POINT_ADDR);
        });
    ...
    describe("ERC-4337 multi-EntryPoint trust (v0.8 + v0.9)", () => {
        ...
        it("isSupportedEntryPoint() is true for v0.8 and v0.9 only; entryPoint() reports v0.9", async () => {
            const { delegate } = await setup();
            ...
            expect(await delegate.entryPoint()).to.equal(ENTRY_POINT_V9_ADDR);
        });
